...
This directory contains GitHub-specific configuration files for the Cohortix project, implementing the CI/CD pipeline architecture defined in **Axon Codex v1.1 §4.8**.
...
The pipeline follows a **two-stage approach** optimized for fast feedback and thorough validation:

### Stage 1: Commit Stage (2-5 min target)
**Triggered:** Every push to `main`, every PR
**Purpose:** Fast feedback on basic quality checks
...
**Triggered:** Pull requests to `main`
...
**Triggered:** Push to `main` (after PR merge)
...
**How it works:**
- When a PR touches files in a path, GitHub auto-requests reviews from the owners
...
**Solution:** Run `pnpm lint:fix` locally

### CI Failing on Type Check
**Problem:** TypeScript errors
**Solution:** Run `pnpm type-check` locally and fix errors

### CI Failing on Tests
**Problem:** Test failures
**Solution:** Run `pnpm test` locally, fix failing tests
...
**Solution:** Add more tests or adjust threshold in `ci.yml` line 144
