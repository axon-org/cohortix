...
Ensure consistent dev-mode auth bypass across ALL API routes to enable QA testing without manual authentication headers.
...
**Important:** This ONLY works in development. Production enforces full authentication via Supabase RLS.
...
1. **Service Role vs RLS Client:**  
   Must use service role client (`SUPABASE_SERVICE_ROLE_KEY`) when bypassing auth, not the regular `createClient()` which enforces RLS policies.

2. **Helper Functions with RLS:**  
   Functions like `getCohortById()` use RLS-protected clients. In bypass mode, either:
...
3. **Consistency is Critical:**  
   All routes must implement bypass the same way. Mixed patterns lead to 401/403 errors and wasted debugging time.
...
All API routes now support dev-mode auth bypass. QA team can test all endpoints without manual authentication headers.
