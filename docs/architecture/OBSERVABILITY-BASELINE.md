...
This document defines the **minimum viable observability** for Cohortix, following Codex standards for structured logging, RED metrics, and distributed tracing.
...
**Philosophy:** Start simple (logs + basic metrics), add complexity as the system grows.
...
**Every HTTP request MUST have a unique correlation ID** to trace its flow through the system.
...
export async function middleware(request: NextRequest) {
  const correlationId = request.headers.get('x-correlation-id') || `req_${uuidv4()}`;
...
**Recommended Library:** [Pino](https://getpino.io/) (fast, JSON-native, low overhead)
...
**Alternative (Simpler):** Application-level logging + log aggregation (e.g., Vercel Analytics, DataDog)
...
*Document maintained by: Guardian (Hafiz)*  
*Next review: 2026-03-01 (Monthly observability review)*
