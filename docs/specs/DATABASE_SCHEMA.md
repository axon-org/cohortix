...
> Comprehensive PostgreSQL schema design with multi-tenant isolation and vector
> storage
...
PostgreSQL RLS policies enforce isolation using Supabase's `auth.uid()`
function.
...
**Note:** User authentication is handled by Supabase Auth. The `auth.users`
table is managed by Supabase and referenced via foreign keys.
