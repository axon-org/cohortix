...
2. **`server.ts`** - Server-side client with cookie handling for server
   components
...
- Added proper type annotations for `cookiesToSet` parameters to resolve
  TypeScript errors
...
- Changed import from `@cohortix/database/supabase` to `@repo/database/supabase`
  to match the workspace naming convention
...
- Note: Peer dependency warning for `lucide-react` with React 19 (unrelated to
  this migration)
...
- If you were using Clerk's user metadata, migrate to Supabase's `auth.users` or
  custom user table
...
1. **Peer Dependency Warning**: `lucide-react` shows peer dependency warning
   with React 19. This is unrelated to the migration and can be safely ignored
   until lucide-react updates their peer dependencies.

2. **Cookie Options Type**: Used `any` type for cookie options as they're passed
   through from the framework. This is acceptable as the types are validated by
   the underlying libraries.

3. **Middleware Matcher**: The middleware excludes static assets but protects
   all other routes. Adjust the matcher pattern in `apps/web/src/middleware.ts`
