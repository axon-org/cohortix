...
This migration implements **Option A** (full database-level security) for
Clerk-compatible Row Level Security (RLS) on Cohortix's core tenant tables:
...
- Helper functions (`get_current_clerk_user_id`, `is_service_role`,
  `set_config`)
...
Replace `createServiceClient()` usage with `createRLSClient()` for user
operations:
