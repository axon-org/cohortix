...
**Date:** 2026-02-14 **Audited by:** Nina (QA Engineer) **Total Failing:** 14
test files (14 individual failures)
...
**Why:** Vitest is trying to run Playwright E2E tests. The `vitest.config.ts`
excludes `'e2e'` but tests are in `apps/web/e2e/` and Vitest is still picking
them up.

**Fix:** Update `vitest.config.ts` exclude pattern to properly exclude E2E
tests:
...
**Dependencies:** None **Blocked by type fixes:** No
...
**Failure 1:** `should require DATABASE_URL environment variable` **Error:**
`expected undefined to be defined`

**Why:** Test expects process.env.DATABASE_URL to be undefined (to test error
handling), but setup.ts sets it
...
**Failure 2:** `should use correct connection string format` **Error:**
`Invalid URL`

**Why:** Test tries to parse DATABASE_URL with `new URL()` but the test value
from setup.ts causes error
...
**Error:**
`expected "vi.fn()" to be called with arguments: [ 'https://test.supabase.co', 'test-anon-key' ] Received: [ undefined, undefined ]`

**Why:** The mock isn't being set up properly, createBrowserClient is being
called with undefined values

**Fix:** Fix the mock setup in the test file

**Failure 2:** `should have required environment variables` **Error:**
`expected undefined to be defined`

**Why:** Test expects env vars to be set, but something is overriding the
setup.ts values

**Fix:** Check test isolation and ensure env vars are properly set

**Failure 3:** `should use HTTPS for Supabase URL` **Error:**
...
**Fix:** Add `import React from 'react'` or update Vitest config to auto-import
React
...
**Test:** `should only log debug in development mode` **Error:**
`TypeError: 'process.env' only accepts a configurable, writable, and enumerable data descriptor`

**Why:** Test tries to modify process.env.NODE_ENV directly with
`Object.defineProperty()`, but Vitest doesn't allow this
...
3. **engagement-timeline React import** (fixes 8 test failures) - Add React
   import
...
**All 14 test failures can be fixed independently** - None are blocked by Devi's
type fixes.
