...
**Created:** 2026-02-14  
**Status:** Draft  
**Purpose:** Comprehensive audit to ensure all pages display real DB data, not
...
Ahmad reports "dummy/hardcoded data" but code review shows **all APIs are
properly wired**. Most likely causes:
...
- **John (backend-developer)**: Fix auth bypass, RLS policies, seed organization
  data
...
- **Devi (ai-developer)**: Verify all dashboard APIs return real data, add error
  handling
...
- **Devi (ai-developer)**: Verify mission-operation linking, progress
  calculation
