...
## Goal
Establish the primary workspace for managing execution (Kanban) and the foundation for organizational memory (Intelligence).
...
- **[UI]** Interactive Kanban board with columns: Todo, In Progress, Review, Done. (Lubna)
- **[UI]** Drag-and-drop support for status transitions using `dnd-kit`. (Lubna)
- **[BE]** API endpoints for bulk status updates and position persistence. (Devi)
...
- **[BE]** Advanced filtering API (by Domain, Vision, Mission, Ally, Status). (Devi)
...
- **[Creative]** "Intelligence Zone" visual identity and empty state illustrations. (Zara)
