...
- **Type checking:** TypeScript strict (`pnpm type-check`)
  - **Do not** run `npx tsc --noEmit` from the repo root — it does not include
    the `scripts/` directory.
  - The `scripts/` directory has its own `scripts/tsconfig.json`. To type-check
    it standalone: `./node_modules/.bin/tsc --noEmit -p scripts/tsconfig.json`
