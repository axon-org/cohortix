...
async function verify() {
  ...
  const { data: members, error: membersError } = await supabase.from('cohort_members').select('*');
  ...
  if (membersError) {
    ...
    if (members.length > 0) {
      ...
      members
        ...
        .forEach((m) =>
          console.log(
            `     - cohort_id=${m.cohort_id.slice(0, 8)}... agent_id=${m.agent_id.slice(0, 8)}... engagement=${m.engagement_score}`
          )
        );
    }
  }
  ...
  // Check audit_logs
  const { data: audits, error: auditsError } = await supabase.from('audit_logs').select('*');
  ...
  if (auditsError) {
    ...
    if (audits.length > 0) {
      ...
      audits
        .slice(0, 3)
        .forEach((a) => console.log(`     - ${a.action} ${a.resource_type} by ${a.actor_type}`));
    }
  }
