...
test.describe('Authentication Flow', () => {
  test.beforeEach(async ({ page, browserName }) => {
    test.skip(browserName !== 'chromium', 'Auth provider rate-limits parallel multi-browser CI runs')
    ...
    try {
      ...
      await expect(page.locator('.cl-rootBox, .cl-signIn-root, form.cl-form, input[name="identifier"]')).toBeVisible({ timeout: 15000 })
    } catch (e) {
  ...
  test('should navigate to sign-up page', async ({ page }) => {
    ...
    await expect(page.locator('input[name="emailAddress"], input[name="identifier"]')).toBeVisible()
  })

  test('should navigate to forgot password page', async ({ page }) => {
    // Attempt to find forgot password link. It might be hidden in step 1.
    const resetLink = page.getByRole('link', { name: /forgot password|reset password|forgot/i }).first()
  ...
  test('sign-in form should be keyboard navigable', async ({ page }) => {
    ...
    const focusedName = await page.evaluate(() => document.activeElement?.getAttribute('name') || document.activeElement?.textContent)
...
test.describe('Sign Up Flow', () => {
  test.beforeEach(async ({ page, browserName }) => {
    ...
    await expect(page.locator('.cl-rootBox, .cl-signUp-root, form.cl-form')).toBeVisible({ timeout: 10000 })
  })
