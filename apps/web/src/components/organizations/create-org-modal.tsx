      ...
      <DialogContent className="sm:max-w-[480px] p-0 overflow-hidden bg-card border-border shadow-2xl">
        <div className="p-8 space-y-8">
          <DialogHeader className="space-y-3">
            ...
            <DialogTitle className="text-2xl font-bold tracking-tight">Create Organization</DialogTitle>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="org-name" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Organization Name</Label>
                <Input 
                  id="org-name" 
                  value={name} 
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label htmlFor="org-slug" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Workspace URL</Label>
                  <Badge variant="outline" className="text-[10px] font-mono h-4 border-dashed bg-muted/10 opacity-60">clerk.org_id sync</Badge>
                </div>
                ...
                <p className="text-[10px] text-muted-foreground flex items-center gap-1.5 italic font-medium px-1">
                  <Info className="h-3 w-3 text-primary/60" /> Your team members will join using this unique workspace slug.
                </p>
              </div>
            </div>

            <DialogFooter className="pt-4">
              <Button type="button" variant="ghost" onClick={() => onOpenChange(false)} disabled={isPending} className="font-semibold text-xs tracking-tight">Cancel</Button>
              <Button type="submit" disabled={isPending || !name || !slug} className="min-w-[120px] font-bold text-xs tracking-widest uppercase">
