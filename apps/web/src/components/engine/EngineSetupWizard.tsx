...
export function EngineSetupWizard({ cohortId }: { cohortId: string }) {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return <Step3Connectivity data={data} updateData={updateData} onNext={nextStep} onBack={prevStep} />;
      case 3:
        return <Step4Credentials data={data} updateData={updateData} onNext={nextStep} onBack={prevStep} />;
      case 4:
            ...
            <li key={step.id} className={cn(stepIdx !== STEPS.length - 1 ? 'pr-8 sm:pr-20' : '', 'relative')}>
              {stepIdx < currentStep ? (
                ...
                <>
                  ...
                  <a href="#" className="group relative flex h-8 w-8 items-center justify-center rounded-full border-2 border-muted bg-background hover:border-gray-400">
