...
import { WizardData, Agent } from "./types";
...
export function Step6DiscoverAgents({ 
  ...
  const [agents, setAgents] = useState<Agent[]>([
    ...
    { id: 'researcher', name: 'Researcher', workspace: '~/.openclaw/workspace-researcher', status: 'online' },
  ]);
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          {agents.map((agent) => (
            <div key={agent.id} className="flex items-start space-x-3 p-4 border rounded-lg hover:bg-muted/50 transition-colors">
              <Checkbox 
                id={agent.id} 
              ...
              <div className="grid gap-1.5 leading-none">
                ...
                >
                  {agent.name}
                  <span className="ml-2 text-xs text-muted-foreground font-normal">({agent.id})</span>
                </Label>
