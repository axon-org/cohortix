...
export function EvolutionEventCard({ event }: EvolutionEventCardProps) {
  ...
  return (
    <div className="relative pl-6 pb-6 last:pb-0">
      <div
        className={cn(
          'absolute left-[-1.3rem] top-1.5 h-3 w-3 rounded-full border-4 border-card',
          config.dotColor
        )}
      />
      ...
      <Card className="bg-card border-border hover:border-muted transition-colors">
        <CardContent className="p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className={cn(
                  'text-[10px] font-bold uppercase py-0.5 px-2 tracking-wider gap-1.5',
                  config.color
                )}
              >
                {config.icon}
            ...
            {event.metadata?.taskId && (
              <Badge
                variant="secondary"
                className="bg-muted text-[10px] flex items-center gap-1 cursor-pointer hover:bg-muted/80"
              >
          ...
          <div className="flex flex-col gap-1.5">
            <p className="text-sm font-medium tracking-tight leading-relaxed">{event.summary}</p>
