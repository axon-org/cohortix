...
const AVAILABLE_SKILLS: Skill[] = [
  { id: 'web-search', name: 'Web Search', category: 'General', icon: <Globe className="h-3 w-3" /> },
  { id: 'coding-agent', name: 'Coding Agent', category: 'Development', icon: <Code className="h-3 w-3" /> },
  { id: 'database-query', name: 'Database Query', category: 'Backend', icon: <Database className="h-3 w-3" /> },
  { id: 'slack-post', name: 'Slack Post', category: 'Social', icon: <MessageSquare className="h-3 w-3" /> },
    ...
    <Card className="bg-card border-border shadow-sm">
      <CardHeader className="p-5 pb-2">
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-col space-y-1">
            <CardTitle className="text-sm font-bold tracking-tight uppercase">Agent Capabilities</CardTitle>
            <CardDescription className="text-[11px] font-medium text-muted-foreground">Manage which skills are available to this agent.</CardDescription>
          </div>
          <Badge variant="secondary" className="bg-primary/10 text-primary text-[10px] font-bold px-2 py-0.5">
      ...
      <CardContent className="p-5 flex flex-col space-y-4">
        ...
        <div className="pt-2 flex flex-col space-y-2">
          <span className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider">Available Skills</span>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {filteredSkills.filter(s => !enabledSkills.includes(s.id)).map(skill => (
              <button
                key={skill.id}
