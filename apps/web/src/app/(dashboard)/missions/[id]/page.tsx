  ...
  if (error || !mission) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/missions" className="text-sm text-foreground underline mt-2 inline-block">Back to Missions</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <Link href="/missions" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="bg-card border border-border rounded-lg divide-y divide-border">
        <Field label="Description">
          {editing ? (
            ...
            <p className="text-sm text-muted-foreground">{mission.description || 'No description'}</p>
          )}
        </Field>
        <Field label="Color">
          {editing ? (
            ...
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded" style={{ backgroundColor: mission.color || '#5E6AD2' }} />
        ...
        <Field label="Start Date">
          {editing ? (
            <Input type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} className="w-48" />
          ) : (
        ...
        <Field label="Target Date">
          {editing ? (
            <Input type="date" value={form.targetDate} onChange={(e) => setForm({ ...form, targetDate: e.target.value })} className="w-48" />
          ) : (
        ...
        {mission.completed_at && (
          <Field label="Completed">
            <p className="text-sm text-success">{new Date(mission.completed_at).toLocaleDateString()}</p>
          </Field>
        )}
        <Field label="Created">
          <p className="text-sm text-muted-foreground">{new Date(mission.created_at).toLocaleDateString()}</p>
        </Field>
      </div>
    </div>
  )
