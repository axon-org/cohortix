...
describe('Cohort Validation Schemas', () => {
  ...
  describe('createCohortSchema', () => {
    ...
    it('should reject name less than 3 characters', () => {
      expect(() =>
        createCohortSchema.parse({ ...validData, name: 'AB' })
    ...
    it('should reject name more than 255 characters', () => {
      ...
      expect(() =>
        createCohortSchema.parse({ ...validData, name: longName })
      ).toThrow(/less than 255 characters/)
    })
    ...
    it('should reject invalid date format', () => {
      expect(() =>
        createCohortSchema.parse({ ...validData, startDate: 'invalid-date' })
  ...
  describe('updateCohortSchema', () => {
    ...
    it('should validate memberCount', () => {
      expect(() => updateCohortSchema.parse({ memberCount: -1 })).toThrow(
        /cannot be negative/
      )
      expect(() => updateCohortSchema.parse({ memberCount: 1.5 })).toThrow(
        /must be an integer/
      )
    })

    it('should validate engagementPercent range', () => {
      expect(() => updateCohortSchema.parse({ engagementPercent: -1 })).toThrow(
        /at least 0%/
      )
      ...
      expect(() =>
        updateCohortSchema.parse({ engagementPercent: 50 })
      ).not.toThrow()
    })
  ...
  describe('cohortQuerySchema', () => {
    ...
    it('should accept valid sortBy values', () => {
      expect(() =>
        cohortQuerySchema.parse({ sortBy: 'name' })
      ).not.toThrow()
      expect(() =>
        cohortQuerySchema.parse({ sortBy: 'memberCount' })
      ).not.toThrow()
      expect(() =>
        cohortQuerySchema.parse({ sortBy: 'engagementPercent' })
      ).not.toThrow()
    })

    it('should reject invalid sortBy', () => {
      expect(() =>
        cohortQuerySchema.parse({ sortBy: 'invalidField' })
      ).toThrow()
    })
  ...
  describe('healthTrendsQuerySchema', () => {
    ...
    it('should accept valid period values', () => {
      expect(() =>
        healthTrendsQuerySchema.parse({ period: '30d' })
      ).not.toThrow()
      expect(() =>
        healthTrendsQuerySchema.parse({ period: '90d' })
      ).not.toThrow()
      expect(() =>
        healthTrendsQuerySchema.parse({ period: '1y' })
      ).not.toThrow()
    })

    it('should reject invalid period', () => {
      expect(() =>
        healthTrendsQuerySchema.parse({ period: '7d' })
      ).toThrow()
    })

    it('should accept valid interval values', () => {
      expect(() =>
        healthTrendsQuerySchema.parse({ interval: 'day' })
      ).not.toThrow()
      expect(() =>
        healthTrendsQuerySchema.parse({ interval: 'week' })
      ).not.toThrow()
      expect(() =>
        healthTrendsQuerySchema.parse({ interval: 'month' })
      ).not.toThrow()
    })

    it('should reject invalid interval', () => {
      expect(() =>
        healthTrendsQuerySchema.parse({ interval: 'year' })
      ).toThrow()
    })
  })
})
