...
describe('Cohort CRUD API - Integration Tests', () => {
  ...
  describe('GET /api/cohorts', () => {
    ...
    it('should apply status filter', async () => {
      ...
      expect(response.status).toBe(200)
      expect(mockGetCohorts).toHaveBeenCalledWith('org-123', expect.objectContaining({
        status: 'active',
      }))
    })
  ...
  describe('POST /api/cohorts', () => {
    ...
    it('should handle duplicate cohort names gracefully', async () => {
      ...
      mockCreateCohort.mockRejectedValue(new Error('duplicate key value violates unique constraint'))
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle null values in optional fields', async () => {
      ...
      mockCreateCohortSchema.parse.mockReturnValue({ name: 'Test Cohort', status: 'active' } as any)
      mockCreateCohort.mockResolvedValue(mockCohort as any)
    ...
    it('should handle special characters in cohort name', async () => {
      ...
      mockCreateCohortSchema.parse.mockReturnValue({ ...specialNameInput, status: 'active' } as any)
