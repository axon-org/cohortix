...
Sprint 4 introduces the Mission Control feature with Missions, Operations, and enhanced PPV hierarchy support. While the core implementation is functional, several critical issues were identified that require attention before production deployment.
...
Each route file has its own `getAuthContext()` function instead of using the centralized helper:
...
missionId: uuid('goal_id').references(() => missions.id, { onDelete: 'set null' })
```

**Issue:** This is confusing and error-prone. The API should use consistent naming.
...
**Finding:** Multiple tables have Drizzle schema definitions that don't match Supabase migrations.
...
**Impact:** CRITICAL — The application will fail if it tries to access these columns.
...
**Migration:** No separate `cohort_members` table definition found in migrations. The migration creates it inline:
...
**Impact:** HIGH — Currently allows any authenticated user to access ANY organization's cohorts.

---

### 2.2 Operations Query File (`apps/web/src/server/db/queries/operations.ts`)

**✅ Was previously fixed** for the invalid `agents!projects_owner_id_fkey` join issue.
...
**However**, it describes a centralized auth helper pattern that isn't consistently implemented:
...
**Recommendation:** Either update the documentation to match reality, or refactor routes to use the centralized helper.
...
const { data: operations, error, count } = await withRetry(
  () => queryBuilder,
...
**Recommendation:** Add CI check to fail if `BYPASS_AUTH=true` in non-development builds.
